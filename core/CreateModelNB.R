...
CreateModelNBClassification = function(data,dataVal,dataMin,dataMax,varType,splitPoints,
                                       maxDegree,verbose=FALSE,plots=FALSE)
{
  # it creates the Naive-Bayes model for classification with a discrete class using data 
  ...
  prob[[length(prob)]] = DiscreteProb(data[,ncol(data)],classVal,verbose,plots)[,2];
  
  # the links matrix has ones only in the row of the class
  links = matrix(0,nrow=ncol(data),ncol=ncol(data));
  links[ncol(data),-ncol(data)] = 1;
...
CreateModelNBRegression = function(data,dataVal,dataMin,dataMax,varType,splitPoints,
                                   maxDegree,verbose=FALSE,plots=FALSE)
{
  # it creates the Naive Bayes model for regression with a continuous class using data 
  # (the selected variables in each case). It contains the conditional probabilities
  # in discrete variables and the conditional polynomials in continuous variables
  # INPUT: - data = a data-frame with the data of study
  #        - dataVal = list with the different values of the discrete variables
  ...
  # matrix with the probabilities of C (last position)
  adjustClass = Adjust1ParentCD(C,classMin,classMax,rep(0,length(C)),0,ncol(data),maxDegree,
                                verbose,plots);
  ...
  for (i in 1:(ncol(data)-1))
  {
    ...
    if (varType[i]==0)   # when the variable is discrete
    {
      ...
      # calculate the conditional probabilities and the values
      p = Adjust1ParentDC(data[,i],dataVal[[i]],C,classMin,classMax);
    ...
    else if (varType[i]==1)   # when the variable is continuous
    {
      ...
      # calculate the polynomials conditioned to the class
      p = Adjust1ParentCC(data[,i],dataMin[i],dataMax[i],C,classMin,classMax,i,maxDegree,
                          verbose,plots);
