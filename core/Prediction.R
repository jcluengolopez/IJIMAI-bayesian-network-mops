...
Predict = function(prob,data,verbose=FALSE,plots=FALSE)
{
  # given the model and one data test, it returns the predictions
  # INPUT: - prob = the TAN regression or classification model (in the user's mode)
  #        - data = the data-test (without the class)
  #        - verbose = show the 'cats' on screen
  #        - plots = draw the graphs
  # OUTPUT: - prediction = predicted observations
  
  
  # create the vector for the predictions
  prediction = vector('numeric',length=dim(data)[1]);
  
  # change the structure of the model
  prob2 = StructureModelIverse(prob);
  
  # type of variables and names
  varType = prob2[[1]];
  
  # separate classification and regression
  if (varType[length(varType)]==0)       # classification
  {
    # for each new observation
    for (i in 1:length(prediction))
    {
      # do the prediction of the class
      prediction = PredictDataClassification(prob2,data);
    }
  }
  else         # regression
  {
    ...
    {
      # do the prediction of the class
      prediction = PredictDataRegression(prob2,data);
    }
  }
...
TestModelClassification = function(prob,data,verbose=FALSE,plots=FALSE)
{
  # given the model and the data-test, it calculates the % of well-classified observations
  # INPUT: - prob = the TAN model
  #        - data = the data-test
  #        - verbose = show the 'cats' on screen
  #        - plots = draw the graphs
  # OUTPUT: - correct = percentage of well classified elements of the data-test
  #         - errors = contingency table with the errors
  
  
  # predict the datatest
  prediction = PredictDataClassification(prob,data[,1:(ncol(data)-1)]);
  
  # check the well classified observations
  test = (prediction == data[,dim(data)[2]]);
  
  # calculate the percentage of correct estimations in the data.test
  correct = length(test[test==TRUE])/(length(test));
  
  # add all the possible values of the class to both vectors
  classVal = prob[[length(prob)-1]];
  prediction = c(prediction,classVal);
  real = c(data[,dim(data)[2]],classVal);
  
  # obtain the contingency table with the errors
  errors = table(prediction,real);
  
  # substract 1 to the diagonal to remove the classVal elements added
  diag(errors) = diag(errors) - 1;
  
  return(list(correct,errors));
}
############################################################
...
TestModelRegression = function(prob,data,verbose=FALSE,plots=FALSE)
{
  # given the model and the data-test, it calculates the mean square error of the 
  # predictions
  # INPUT: - prob = the TAN model
  #        - data = the data-test
  #        - verbose = show the 'cats' on screen
  #        - plots = draw the graphs
  # OUTPUT: - error = mean square error of the predictions
  
  
  # create a vector for each data of the test
  error = vector('double',length=nrow(data));
  
  # predict the observations
  prediction = PredictDataRegression(prob,data[,1:(ncol(data)-1)],verbose,plots);
  
  # difference between the approximation and the real value
  error = (prediction - data[,ncol(data)]) ^ 2;
...
PredictDataClassification = function(prob,data,verbose=FALSE,plots=FALSE)
{
  # it predicts the discrete class of one single observation data using the model
  # INPUT: - prob = the TAN model
    ...
    if (varType[posParent]==1)   # 2 parents (the class and another continuous)
    {
      ...
      else     # discrete or discretized
      {
        ...
        density = vapply(1:length(x), function(i) {
          if (x[i] %in% dataVal)   
          {   
            density = dataMatrix[dataVal==x[i],interval[i],class];
          }
          else   {   density = 0;   }
          return(density)   },  1);
      }
...
PredictDataRegression = function(prob,data,verbose=FALSE,plots=FALSE)
{
  ...
  probXYC = function(x,parent,class,posX,posParent)
  {
    ...
    if (varType[posParent]==1)   # 2 parents (the class and another continuous)
    {
      ...
      #cat('\n padre = ',parent);
      #cat('\n intervalo del padre en el que deber?a estar = \n',intervalParent);
    ...
    else    # 2 parents (the continuous class and another discrete or discretized)
    {
      ...
      #cat('\n variable ',posX,'con padre ',posParent);
      #cat('\n padre = ',parent,'que est? en la posici?n = ',intervalParent);
      #cat('\n clase = ',class);
      #cat('\n prob[[3*posX+1]] = \n ');   print(prob[[3*posX+1]]);
...
PredicttMoPCC = function(data,parent,poly,values,cutPoints,verbose=FALSE,plots=FALSE)
{
  # it calculates the density of a continuous variable with one continuous parent from a
  # dataset
  # INPUT: - data = observations of the variable
  #        - parent = observations of the parent variable
  #        - poly = matrix with the coefficients of the polynomials
  ...
  #        - cutPoints = a vector with the cut points in the discretization of the parent
  #        - verbose = show the 'cats' on screen
  #        - plots = draw the graphs
  # OUTPUT: - prediction = the density of the tMoP for data
  
  
  # function with the probability of the variable with one parent for one observation
  probXC = function(x,parent)
  {
    # INPUT: - x = observations of the concrete variable
    #        - class = one of the possible values of the class
    #        - posX = position of the variable X in the dataframe
    ...
    # select the used interval of the class (class is the mid point of the interval)
    intervalClass = which(parent < cutPoints)[1] - 1;
    
    #cat('\n intervalClass = ',intervalClass)
    #cat('\n cutPoints = ',cutPoints)
    
    # extract the roots, extremes and probabilities of the queues from the matrix values
    roots = values[,3*intervalClass-2];
    extremes = values[,3*intervalClass-1];
    probQueues = values[,3*intervalClass];
    
    # extract the coefficients that are not NA (degree lower than max)
    aux = poly[intervalClass,];
    p = aux[!is.na(aux)];
    
    # predict the polynomial for the exact value
    polyVal = predict(as.polynomial(p),x);
