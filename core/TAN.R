...
TAN = function(data,class,dataVal,dataMin,dataMax,varType,splitPoints,maxDegree,nValidations,
               train,mutualInfoClass,verbose=FALSE,plots=FALSE)
{
  # It makes the TAN model with or without cross validation. Depending on the class, 
  ...
  if (nValidations==0)          # no cross validation and no test
  {
    ...
    {
      # create the model
      prob = CreateModelTANClassification(data,dataVal,dataMin,dataMax,varType,splitPoints,
                                         maxDegree,mutualInfoClass,verbose,plots);
    ...
    {
      # create the model
      prob = CreateModelTANRegression(data,dataVal,dataMin,dataMax,varType,splitPoints,
                                     maxDegree,mutualInfoClass,verbose,plots);
  ...
  else if (nValidations==1)          # no cross validation, but test
  {
    ...
    if (varType[dataDim[2]]==0)       # classification
    {
      prob = CreateModelTANClassification(dataTrain,dataVal,dataMin,dataMax,varType,
                          splitPoints,maxDegree,mutualInfoClass,verbose,plots);
    ...
    {
      # create the model
      prob = CreateModelTANRegression(dataTrain,dataVal,dataMin,dataMax,varType,splitPoints,
                                      maxDegree,mutualInfoClass,verbose,plots);
  ...
  else          # cross validation
  {
    ...
    for (i in 1:nValidations)
    {
      ...
      {
        # create the model
        prob = CreateModelTANRegression(dataTrain,dataVal,dataMin,dataMax,varType,
                            splitPoints,maxDegree,mutualInfoClass,verbose,plots);
